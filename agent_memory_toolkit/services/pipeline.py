...
Phase 2 service extraction: this module owns LLM-call orchestration while
all persistence goes through the injected memory store and all model calls go
through the injected chat client.

Pure helpers (chat-text extraction, transcript building, prompty handling,
LLM JSON parsing) live in :mod:`agent_memory_toolkit.services._pipeline_helpers`
and are shared with :class:`agent_memory_toolkit.aio.services.pipeline.AsyncPipelineService`.
"""
...
class _StoreContainerAdapter:
    """Small compatibility adapter so lifted orchestration uses MemoryStore APIs."""
...
class PipelineService:
    ...
    def _run_prompty(
        ...
        return chat_text(self._chat_client.generate(messages, **params))
